...
# Standartizovani nazvu statu,pridavani weekday a ctvrtleti
...
# Pridani pomoci left joinu dodatecne country z covid19_b_d_view pro lepsi hledani ve finalni tabulce
CREATE TABLE Covid19_tests_performed(	
	SELECT 
	cbd.country,
	cbd.date,
	ct.tests_performed 
	FROM covid19_b_d_view cbd
	LEFT JOIN covid19_tests_view ct 
...
CREATE TABLE economies_view_final(
	SELECT
	cbd.country,
	evg.gdp_per_person,
	average_gini,
	average_mor_under5
FROM covid19_b_d_view cbd
LEFT JOIN economies_view_gdp evg
ON cbd.country = evg.country
LEFT JOIN economies_view ev
...
CREATE TABLE life_expectancy_view ( 
	SELECT
		le1.country,
		ROUND((le1.life_expectancy - le2.life_expectancy),2) AS diff_2015_1956
...
# Promene z tabulky weather do jednoho table
...
SELECT
	...
	evf.gdp_per_person AS gdp_per_person,
	evf.average_gini AS average_gini,
	evf.average_mor_under5 AS average_mor_under5,
	...
	lev.diff_2015_1956 AS diff_2015_1956,
	wv.average_temp AS average_temperature,
	wv.hours AS hours_rain,
	wv.max_wind_speed AS max_wind_speed
FROM covid19_b_d_view cbd
JOIN Covid19_tests_performed ct 
ON cbd.country = ct.country  AND cbd.`date` = ct.`date` 
JOIN countries_view cv
ON cbd.country =cv.country
JOIN economies_view_final evf
ON cbd.country = evf.country
JOIN religions_div rd
ON cbd.country = rd.country 
JOIN life_expectancy_view lev
ON cbd.country = lev.country 
JOIN weather_view wv
ON cbd.country = wv.country AND cbd.`date` = wv.`date`
);
